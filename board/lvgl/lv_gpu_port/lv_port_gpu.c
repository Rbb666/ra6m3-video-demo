...
#if LV_PORT_DISP_GPU_SUB_BYTE_SWAP
 static const lv_color_t *buf;
#endif

 static bool d2_enabled = SCENARIO != ORIGINAL ? 1 : 0;
 static d1_device *d1_handle;
        ...
        const lv_area_t * src_area, d2_u32 flags)
{
        ...
        if (img_dsc.angle == 0) {
                ...
                if (flags & d2_bf_colorize2) {
                        ...
                        D2_EXEC(d2_settexopparam(d2_handle, d2_cc_red, (uint8_t)(cl >> 0UL),
                                                 (uint8_t)(ch >> 0UL)));
                        D2_EXEC(d2_settexopparam(d2_handle, d2_cc_green, (uint8_t)(cl >> 8UL),
                                                 (uint8_t)(ch >> 8UL)));
                        D2_EXEC(d2_settexopparam(d2_handle, d2_cc_blue, (uint8_t)(cl >> 16UL),
                                                 (uint8_t)(ch >> 16UL)));
                }
